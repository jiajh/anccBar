...
//      UIImage *ii = [UIImage imageWithCGImage:rotatedImage];
//      NSLog(@"rotatedimage:%g,%g",ii.size.width,ii.size.height);
...
//      ii = [UIImage imageWithCGImage:rotatedImage];
//      NSLog(@"croppedImage:%g,%g",ii.size.width,ii.size.height);
...
//          [self stop];
        if (result) {
          dispatch_async(dispatch_get_main_queue(), ^{
            ...
            ZXHybridBinarizer *binarizer2 = [[ZXHybridBinarizer alloc] initWithSource:[source invert]];

            if (self.binaryLayer) {
                CGImageRef image2 = [binarizer2 createImage];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0), dispatch_get_main_queue(), ^{
                    self.binaryLayer.contents = (__bridge id)image2;
                    CGImageRelease(image2);
                });
            }

            if (self.delegate) {
                ZXBinaryBitmap *bitmap2 = [[ZXBinaryBitmap alloc] initWithBinarizer:binarizer2];

                NSError *error2;
                ZXResult *result2 = [[[ZXDataMatrixReader alloc] init] decode:bitmap2 hints:[ZXDecodeHints hints] error:&error2];
                if (result2) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self.delegate captureResult:self result:result2];
                    });
                } else {
                     NSLog(@"shibaile");
                }
