...
- (void)setHints:(ZXDecodeHints *)hints {
  ...
  if ([self.readers count] == 0) {
    ...
    [self.readers addObject:[[ZXMaxiCodeReader alloc] init]];
      // 增加汉信码解析器
      [self.readers addObject:[[ZXHanxinReader alloc] init]];
      //TODO ZXMultiFormatUPCEANReader本来是在ZXMultiFormatOneDReader中被添加，但由于汉信码经常错误识别为UPC-E，所以把汉信码识别器放到前面，但即使这样还是有识别错误的可能
